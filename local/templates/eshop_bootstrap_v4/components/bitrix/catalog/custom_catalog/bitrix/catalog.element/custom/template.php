...
$productOrSkuId = \CU_Goods_Helper::getProductOrSkuId($arResult);
?>
		...
		<div class="<?=($showBlockWithOffersAndProps ? "col-md-5 col-lg-6" : "col-md-4"); ?>">
							...
							switch ($blockName)
							{
								case 'rating':
									if ($arParams['USE_VOTE_RATING'] === 'Y')
									{
										...
										<div class="mb-3">
											<?php
											//https://uchitel.planfix.ru/task/468163/?comment=9247109
											//Карточка товара:
											//1. Не показывать рейтинг товара  в карточке, если не было продаж товара
											$wasBought = \CU_Goods_Helper::wasProductBought($arResult['ID']);
											$arParams['WAS_BOUGHT'] = $wasBought;   //чтобы прокинуть в компонент комментариев
											if ($wasBought) {
								...
								case 'buttons':
									...
									<div data-entity="main-button-container" class="mb-3">
										<?\CU_Goods_Helper::printProductDelay($productOrSkuId)?>
										<?\CU_Goods_Helper::printWrongSellerModal()?>
										<?
										$isProductInActualBasket = \CU_Goods_Helper::IsProductInActualBasket($arResult['ID']);
										?>
										<div id="<?=$itemIds['BASKET_ACTIONS_ID']?>" style="display: <?=($actualItem['CAN_BUY'] ? '' : 'none')?>;">
											...
											if ($showBuyBtn)
											{
												...
												$isByPreorder = \CU_Goods_Helper::isByPreorder($arResult['PROPERTIES']);
												if ($isByPreorder) {
												?>
												<div class="js-buy-btn mb-3 <?=$buyClass?>">
												...
												<?}?>
												<div class="js-bought-btn mb-3 <?=$boughtClass?>">
													<a class="btn <?=$buyButtonClassName?> product-item-detail-buy-button"
														id="<?=$itemIds['BUY_LINK']?>"
														href="javascript:void(0);">
			...
			<?if ($arResult['PROPERTIES']['PUBLISHER']['VALUE']) {?>
				<div class="row">
					...
					<?\CU_Deliveries_Helper::printDeliveries($arResult['PROPERTIES']['PUBLISHER']['VALUE']);?>

					<?\CU_Goods_Helper::printSeller($arResult['ID']);?>
				</div>
			<?}?>
		</div>
...
//								'CACHE_TYPE' => $arParams['CACHE_TYPE'],
							...
							$componentCommentsParams['WAS_BOUGHT'] = $arParams['WAS_BOUGHT'];
							$APPLICATION->IncludeComponent(
								'eshop_bootstrap_v4:catalog.comments',
								'',
...
<script>
	//js для событий в карточке--{
    function delay(productId) {
        fetch("/ajax/delay.php", {
            "headers": {
            ...
            function (data) {
                if (data) {
                    ...
                    document.querySelector('.delayed').classList.remove('hidden');

                    var delayedModal = new bootstrap.Modal(document.getElementById('delyaedModalId'));
                    delayedModal.show();
                } else {
                    console.log('отложен товар с айдишником: ' + data)
                }
            }
        );
    }
    ...
    function hasBought() {
        document.querySelector('.js-buy-btn').classList.add('hidden');
        document.querySelector('.js-bought-btn').classList.remove('hidden');

        //обновим количество товаров
        BX.ajax.loadJSON(
            '/ajax/actualize_baskets_count.php',
            function (res) {
	            //todo
	            //$('.header__favorites .badge-inner').text(res.delayed);
	            //$('.header__basket .badge-inner').text(res.actual);
            }
        );
    }
    //js для событий в карточке--}
	//js - производитель
	var jsSellerId = <?=$arResult['PROPERTIES']['PUBLISHER']['VALUE']?>;
	var jsProductId = <?=$productOrSkuId?>;
