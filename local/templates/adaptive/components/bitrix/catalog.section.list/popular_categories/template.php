	...
	<div class="homepage__popular homepage__fp-slide-full-height">
		...
		if ($arResult["SECTIONS"]) {
			...
			<div class="homepage__popular-items">
				...
				$rowCounter = 0;
				for ($i=0; $i<6; $i++) {
					if (!$arResult["SECTIONS"][$i]) {
						continue;
					}
					$section = $arResult["SECTIONS"][$i];
					if ($rowCounter == 0) {
						echo '<div class="homepage__popular-items-row">';
					}
					$rowCounter++;
					?>
						<a href="<?=\CU_Url_Helper::sectionDetail($section['ID'])?>"><img src="<?=$section['PICTURE']['SRC']?>" alt="<?=$section['PICTURE']['ALT']?>"><span><?=$section['NAME']?></span></a>
					<?
					if ($rowCounter == 2) {
						$rowCounter = 0;
						echo '</div>';
				...
				//если их нечётное количество, то надо всё равно закрыть тэг
				if ($rowCounter == 1) {
					echo '</div>';
				}
				?>
